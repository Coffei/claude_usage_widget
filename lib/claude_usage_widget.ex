defmodule ClaudeUsageWidget do
  ...
  defp print_error(:not_found, :short), do: IO.puts(:stderr, "Config not set up")

  defp print_error(:not_found, _mode) do
    IO.puts(:stderr, Config.setup_instructions())
  end

  defp print_error(:invalid_json, :short), do: IO.puts(:stderr, "Error: Invalid JSON in config")

  defp print_error(:invalid_json, _mode) do
    IO.puts(:stderr, "Error: Configuration file contains invalid JSON")
  end

  defp print_error({:missing_field, field}, :short),
  ...
  defp print_error({:empty_field, field}, :short),
  ...
  defp print_error(:unauthorized, :short),
  ...
  defp print_error(:forbidden, :short),
  ...
  defp print_error({:http_error, status, _body}, :short),
  ...
  defp print_error({:request_failed, _exception}, :short),
  ...
  defp print_error({:file_error, _reason}, :short),
